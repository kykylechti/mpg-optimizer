...
def constraint_optimization_inference(players: pd.DataFrame, budget: int = 500):
    ...
    prob += pulp.lpSum([players.loc[i, 'cote'] * player_vars[i] for i in players.index]) <= budget

    prob += pulp.lpSum([player_vars[i] for i in players.index]) == 11

    if "poste_G" in players.columns:
        
        prob += pulp.lpSum([players.loc[i, 'poste_G'] * player_vars[i] for i in players.index]) == 1
        
        prob += pulp.lpSum([players.loc[i, 'poste_D'] * player_vars[i] for i in players.index]) >= 3
        prob += pulp.lpSum([players.loc[i, 'poste_D'] * player_vars[i] for i in players.index]) <= 5
        
        prob += pulp.lpSum([players.loc[i, 'poste_M'] * player_vars[i] for i in players.index]) >= 3
        prob += pulp.lpSum([players.loc[i, 'poste_M'] * player_vars[i] for i in players.index]) <= 5
        
        prob += pulp.lpSum([players.loc[i, 'poste_A'] * player_vars[i] for i in players.index]) >= 1
        prob += pulp.lpSum([players.loc[i, 'poste_A'] * player_vars[i] for i in players.index]) <= 3
